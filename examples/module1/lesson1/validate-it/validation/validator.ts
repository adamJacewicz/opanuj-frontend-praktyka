import { NumericValidationMethod, isValidInteger } from './utils.ts';
import {
...
export function validate(
  ...
  if (!isValidInteger(input)) return MESSAGE_ERROR_INVALID_INPUT;

  const isValidRangeInteger = validators.every((validateFn) =>
    validateFn(Number(input))
  );

  if (!isValidRangeInteger) return MESSAGE_ERROR_INVALID_INT;
